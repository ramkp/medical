...
if ($action == 'addsession') {
    ...
    $returnurl = new moodle_url('/mod/scheduler/view.php', array('what' => 'view', 'id' => $cm->id, 'subpage' => $subpage));
...
if ($slots) {
    ...
    $PAGE->requires->yui_module('moodle-mod_scheduler-delselected', 'M.mod_scheduler.delselected.init', array($delselectedurl->out(false)));
    ...
    /*
      $completeselected=$commandbar->action_link('#', 'completeselected', '', null, 'completeselected');
      $completeselected->formid = 'completeselected';
      $otherbuttons[]=$completeselected;

      $sendselected=$commandbar->action_link('#', 'sendselected', '', null, 'sendselected');
      $sendselected->formid = 'sendselected';
      $otherbuttons[]=$sendselected;

      $printlected=$commandbar->action_link('#', 'printlected', '', null, 'printlected');
      $printlected->formid = 'printlected';
      $otherbuttons[]=$printlected;
     */

    if (has_capability('mod/scheduler:manageallappointments', $context) && $subpage == 'allappointments') {
        $delbuttons[] = $commandbar->action_link(
                new moodle_url($actionurl, array('what' => 'deleteall')), 'deleteallslots', 't/delete', 'confirmdelete');
        $delbuttons[] = $commandbar->action_link(
                new moodle_url($actionurl, array('what' => 'deleteallunused')), 'deleteallunusedslots', 't/delete', 'confirmdelete');
    }
    $delbuttons[] = $commandbar->action_link(
            new moodle_url($actionurl, array('what' => 'deleteunused')), 'deleteunusedslots', 't/delete', 'confirmdelete');
    $delbuttons[] = $commandbar->action_link(
            new moodle_url($actionurl, array('what' => 'deleteonlymine')), 'deletemyslots', 't/delete', 'confirmdelete');

    $commandbar->add_group(get_string('deletecommands', 'scheduler'), $delbuttons);
    //$commandbar->add_group('Other commands', $otherbuttons);
} // end if $slots
