...
//echo "Enrolled status: ".$enrolled."<br>";
    ...
    function get_paypal_refund_button($courseid, $userid)
    {
        ...
        if ($certificate_date == null) {
            // We make refund only if certificates is not yet issued
            $transid = $this->get_paypal_transaction_id($courseid, $userid);
            $list .= "<button class='paypal_refund' data-transid='$transid'>Refund</button>";
        }
        return $list;
    }
        ...
        if ($num > 0) {
            ...
            while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                ...
                if ($certificate_date != null && $certificate_date < $row['pdate'] && $expired == 1) {
                    $list .= "<span class='span8'>Paid by card $" . round($row['psum']) . "&nbsp;(Authorize.net&nbsp;" . date('m-d-Y H:i:s', $row['pdate']) . ") &nbsp; Certificate Renewal Fee ($coursename) &nbsp; <a class='payment_details' id='payment_details_$transactionid' href='#' style='cursor:pointer;font-weight:bold;color:red;' onClick='return false;' data-transactionid='$transactionid'>Details</a></span>";
                } // end if
                else {
                    $list .= "<span class='span8'>Paid by card $" . round($row['psum']) . "&nbsp;(Authorize.net&nbsp;" . date('m-d-Y H:i:s', $row['pdate']) . ") &nbsp; $coursename &nbsp; <a class='payment_details' id='payment_details_$transactionid' href='#' onClick='return false;' style='cursor:pointer;font-weight:bold;color:red;' data-transactionid='$transactionid'>Details</a></span>";
                }
                if ($status == 0) {
                    $prohibit = $this->get_user_roles($userid);
                    if ($prohibit == 0 && ($current_user == 2 || $current_user == 234)) {
